...
Всего в проекте 31 автотест: 22 UI и 9 API.
...
5. Выход из системы и блокировка повторного открытия защищённой страницы.
6. Проверка ролевого доступа администратора.
7. Проверка доступной навигации менеджера.
8. Запрет прямого открытия административных страниц менеджером.
9. Поиск КП: empty state, найденный результат и очистка поиска.
10. Полный UI-флоу создания КП `Pro · RUB`: открытие, валюта и отображение в списке.
11. Наличие калькуляторов `Base · RUB`, `Pro · RUB`, `KZ · KZT` в форме создания.
12. Валидация обязательных полей формы создания КП.
13. Отмена создания КП без добавления записи в список.
14. Открытие страницы dashboard.
15. Создание КП из dashboard.
16. Перемещение номенклатуры в списке и возврат на исходную позицию.
17. Открытие страницы профиля и проверка активной вкладки.
18. Отображение и редактирование личных данных, загрузка и удаление аватара и подписи.
19. Отображение сохранённых настроек менеджера и контактов после перезагрузки страницы.
...
- `tests/ui/auth/auth.spec.js` — авторизация, валидации формы входа и выход из системы;
- `tests/ui/access/roles.spec.js` — навигация и прямой доступ к страницам для ролей `admin` и `manager`;
- `tests/ui/dashboard/proposals.spec.js` — поиск, создание, валидация и отмена создания КП, работа с dashboard;
- `tests/ui/nomenclatures/nomenclatures.spec.js` — drag-and-drop перемещение номенклатуры;
- `tests/ui/profile/profile.spec.js` — просмотр и редактирование профиля, изображения и настройки менеджера.
...
5. `POST/GET/DELETE` загрузка, получение и удаление аватара и подписи с проверкой отсутствия ссылок после удаления.
6. `POST` создание КП для калькуляторов `Base · RUB`, `Pro · RUB`, `KZ · KZT`.
7. `POST` проверка валидации пустого названия КП.
...
- UI автоматизация: 22 функциональных теста с Page Object, компонентами, fixtures и генератором данных.
- API автоматизация: 9 функциональных тестов с Controller → Service → Facade слоем и генератором данных.
