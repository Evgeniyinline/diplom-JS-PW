...
Всего в проекте 21 автотест: 16 UI и 5 API.
...
После успешных API-тестов созданные пользователи автоматически добавляются в очередь очистки и удаляются в `global.teardown.js`.
...
Для UI-тестов менеджер создаётся один раз на worker через фикстуру `managerStorageState`, авторизуется через API и переиспользуется в тестах через фикстуру `managerApp`. После завершения worker менеджер удаляется.
...
- UI автоматизация: 16 функциональных тестов с Page Object, компонентами, fixtures и генератором данных.
- API автоматизация: 5 функциональных тестов с Controller → Service → Facade слоем и генератором данных.
